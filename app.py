...
def addstudent():
    if request.method=='POST':
        ...
        cursor=mysql.connection.cursor()
        cursor.execute('insert into students(Id,Name,Section,Roomno,moblie) values(%s,%s,%s,%s,%s)',[id1,name,section,room,mobile])
        mysql.connection.commit()
        cursor.close()
...
def update(id):
    ...
    if request.method=='POST':
        ...
        mobile=request.form['mobile']
        cursor.execute('update students set Name=%s,Section=%s,Roomno=%s,moblie=%s where id=%s',[name,section,room,mobile,id1])
        mysql.connection.commit()
        return redirect(url_for('studentrecord'))
...
def checkin():
    ...
    if request.method=='POST':
        ...
        if Id=="" or Name=="" or section=="" or roomno=="" or mobileno=="":
            ...
            cursor=mysql.connection.cursor()
            cursor.execute('insert into records(Id,Name,Section,Roomno,moblieNumber,date) values(%s,%s,%s,%s,%s,%s)',[Id,Name,section,roomno,mobileno,date_today])
            mysql.connection.commit()
            cursor.execute('SELECT * from records')
            std_records=cursor.fetchall()
            cursor.close()
...
@app.route('/allroomsinfo',methods=['GET','POST'])
def allrooms():
    cursor=mysql.connection.cursor()
